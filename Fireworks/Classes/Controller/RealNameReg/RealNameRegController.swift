...
class RealNameRegController: BaseViewController, UITableViewDelegate, UITableViewDataSource,ParameterDelegate,MultiParameterDelegate {
    ...
    func deleteRecord(){
        self.alertNotice("提示", message: "是否删除？") {
            ...
            parameters["fireworkRecord.id"] = String(self.saleRecordModel.id)
...
//        self.navigationController?.navigationBar.shadowImage = UIImage(named: "head_transparent")
    ...
    func saveInfo(){
        ...
        if AppTools.isEmpty(String(strNum)) && AppTools.isEmpty(String(strCarton)){
        ...
        var parameters = [String : AnyObject]()
        if saleRecordModel.id != -1 {
            parameters["fireworkRecord.id"] = saleRecordModel.id
        }
        parameters["type"] = TYPE_CODE
        parameters["fireworkRecord.credentials"] = saleRecordModel.credentials
        parameters["fireworkRecord.credentialsCode"] = strDocumentNum//证件编号
        parameters["fireworkRecord.name"] = strName//姓名
        parameters["fireworkRecord.address"] = strAddress//住址
        parameters["fireworkRecord.companyName"] = strUnit//所在单位
        parameters["fireworkRecord.phone"] = strPhone//联系电话
        parameters["fireworkRecord.productName"] = strProductName//品名
